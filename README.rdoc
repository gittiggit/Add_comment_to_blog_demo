...
Step 34: Add comment to articles
         a. rails g model Comment user_id:integer article_id:integer content:text
         b. rails g controller Comments
         c. Design the UI in the article show



-----------------------------------------------
         To have the collapse content
         <div>
           <button type="button" class="btn btn-info btn-sm pull-right" data-toggle="collapse" data-target="#demo">
           Comments
           </button>
         </div>
         </div>
...
----------------------------------------------------------------------
Step 35: Pdf generation using pdfkit
       a. gem "wkhtmltopdf"
          gem "wkhtmltopdf-binary"
       or
       gem install PDFKit
       b. Next we need to add the rake middleware, and to do that in the config/application.rb

        config.middleware.use "PDFKit::Middleware", :print_media_type => true

       c. To see all the middleware in your app
       => rake middleware

       #d. install the dependent library
       # =>  gem install wkhtmltopdf-binary

       e. Next create a file config/initializers/pdfkit.rb, add the codes
...
Step 36: export to excel/csv
        a. first in the concig/application.rb, add the following lines
          require "csv"

        b. to print all the products, in the index action
        ...
        d.Now to create excel file, in the initializers/mime_types.rb, add the

          Mime::Type.register "application/xls", :xls

        f. Add an option to the index action while calling the to_csv method
           format.xls {send_data @products.to_csv(col_sep: "\t")}

        g. Now add the option to the to_csv method in the product.rb model

***Creating a seperate template for xls file
       a. Create a file views/products/index.xls.erb
